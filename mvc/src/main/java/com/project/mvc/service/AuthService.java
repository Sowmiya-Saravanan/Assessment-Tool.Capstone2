    ...
    public String getEmailFromToken(String jwtToken) {
        try {
            ...
            ResponseEntity<Map<String, String>> response = restTemplate.exchange(
                backendUrl + "/api/educator/validate",
                HttpMethod.POST,
                entity,
                new ParameterizedTypeReference<Map<String, String>>() {}
            );

            if (response.getStatusCode() == HttpStatus.OK && "valid".equals(response.getBody().get("status"))) {
                return response.getBody().get("email");
            } else {
                logger.error("Failed to validate token: {}", response.getBody().get("message"));
                throw new RuntimeException("Failed to validate token: " + response.getBody().get("message"));
            }
        } catch (Exception e) {
